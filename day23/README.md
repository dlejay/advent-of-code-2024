...
Since removing a node from a complete graph still gives you a complete graph, I
used a recursive approach based on computing the size of a maximal clique inside
a graph.
This function is then used again to find the maximal clique. The algorithm is
made computationally bearable with a cache.
