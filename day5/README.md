...
Because of what is expected in Part Two, there shall be only one correct way of ordering the pages.
So one could assume that the list of rules is actually giving us a bijection to a integer segment [0,n].
In that case, the exercice would be to encode the bijection and use a regular sorting algorithm.
The small example here is actually misleading since the final input data is not representing a bijection!
So the trick here is to intersect the rules with the actual elements present in the update.
This gives a bijection every time.
