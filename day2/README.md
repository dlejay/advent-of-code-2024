...
## Notes
- Part 1: one only needs to parse the data once and the monotony of the sequence can easily be checked against the first two terms of the report.
- Part 2: this is trickier. One could try to remove each level and see if the resulting report becomes safe but this leads to `O(n^2)` complexity.
If instead, one chooses to parse the report for potential errors and then assess if it is fixable, then `O(n)` can be achieved.
Since only a single bad level is tolerated, one can infer the expected monotony of the sequence by looking at its first 4 elements.
